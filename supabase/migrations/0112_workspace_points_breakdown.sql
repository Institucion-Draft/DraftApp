-- 0112_workspace_points_breakdown.sql
--
-- v_workspace_points_breakdown: fuente de datos de la pantalla nueva "Detalle de puntos"
-- (Ranking Global → tocar un jugador) — una fila por evento donde el usuario sumó puntos
-- (event_name, player_count, position, points), en vez del total agregado que ya da
-- v_workspace_points. Misma lógica exacta que las 3 ramas de v_workspace_points (0109/0111):
-- bracket_rows (bracket real de Top4, round_robin_topcut/swiss_topcut), champion_rows (1° de
-- round_robin sin top4) y placement_rows (2°/3° de round_robin sin top4, vía
-- v_rr_no_top_regular_rank ya corregida por winrate + disputa real). SUM(points) de esta vista
-- para un (user_id, workspace_id) debe dar exactamente lo mismo que v_workspace_points.points —
-- se construye a propósito reusando los mismos filtros/joins que esa vista, no una reimplementación
-- paralela, para que ambas no puedan divergir con el tiempo.
...
select user_id, workspace_id, event_id, event_name, player_count, position, points from bracket_rows
union all
select user_id, workspace_id, event_id, event_name, player_count, position, points from champion_rows
union all
select user_id, workspace_id, event_id, event_name, player_count, position, points from placement_rows;
