-- 0046_fix_swiss_bo2_bye.sql
-- Fix: generate_swiss_bo2_round debe asignar bye cuando hay número impar de jugadores,
-- replicando exactamente la lógica de generate_swiss_round (0032/0033) pero usando
-- swiss_bo2_points_of para ordenar/emparejar. Recalcula tiebreakers BO2 al final.
...
begin
  ...
  -- Si hay número impar, asignar bye al jugador con menos puntos que aún no haya recibido bye.
  if v_has_odd then
  ...
  -- Resto de jugadores ordenados por puntos descendente (bye excluido).
