-- 0070_exclude_bo1_top4_from_champion_trigger.sql
-- compute_event_champion() (disparada por on_pairing_official_close_update_champion en
-- cada pairing que resuelve su ganador oficial) no tenía ningún chequeo de
-- competition_format: para un evento round_robin_bo1_top4, al cerrarse el último pairing
-- de la fase regular evaluaba winrate BO1 con el umbral pensado para BO3 y podía setear
-- champion_user_id + status='completed' directo (o crear un event_tiebreak_group propio
-- con group_origin='tiebreak'), todo esto ANTES o EN LUGAR de que exista el bracket de
-- top4 (create_round_robin_top4_bracket, disparado desde el cliente).
-- round_robin_bo1_top4 tiene su propio flujo de cierre vía el bracket de top4: la función
-- no debe hacer nada para este formato, en ningún punto.
