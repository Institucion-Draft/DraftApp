...
export default function PlayerProfileInEventScreen({ route, navigation }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    if (!tgRes.error && tgRes.data?.id) {
      ...
      if (isBracketSourced) {
        ...
        const isMember = bmRows.some(
          (bm) => bm.participant_a_id === participantId || bm.participant_b_id === participantId
        );
    ...
    if (!fpGroupRes.error && fpGroupRes.data?.id) {
      ...
      const fpBmRows = (fpBmRes.data ?? []) as BracketBmRow[];
      const isFpMember = fpBmRows.some(
        (bm) => bm.participant_a_id === participantId || bm.participant_b_id === participantId
      );

      if (isFpMember) {
        setProfileInFourthPlaceGroup(true);
        const semiRows: OfficialH2HRow[] = [];
        const finalRows: OfficialH2HRow[] = [];
        for (const bm of fpBmRows) {
          let oppId: string | null = null;
          if (bm.participant_a_id === participantId) oppId = bm.participant_b_id;
          else if (bm.participant_b_id === participantId) oppId = bm.participant_a_id;
          else continue;
        ...
        setProfileFourthPlaceRows({ semi: semiRows, final: finalRows });
      } else {
  ...
  const renderOfficialContent = () => {
    ...
    const fourthPlaceMataSection = () => {
      if (!profileInFourthPlaceGroup) return null;
...
const createStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    name: { marginTop: 14, fontSize: 22, fontWeight: '800', color: c.text, textAlign: 'center' },
    ...
    profileStreakWin: { backgroundColor: c.status.success.solid },
    profileStreakLoss: { backgroundColor: c.status.error.solid },
    profileStreakDotTxt: { color: c.onAccent, fontSize: 10, fontWeight: '700' },
    profileStreakEmpty: { color: c.textSecondary, fontSize: 11, fontWeight: '600' },
    ...
    leftEventChipText: { fontSize: 11, fontWeight: '700', color: c.textSecondary },
    orgChip: {
      paddingHorizontal: 10,
      paddingVertical: 4,
      borderRadius: 999,
      backgroundColor: c.organizer.subtle,
      borderWidth: 1,
      borderColor: c.organizer.border,
    },
    orgChipTxt: { fontSize: 12, fontWeight: '700', color: c.organizer.text },
    editColorsBtn: {
    ...
    editColorsBtnTxt: { color: c.onAccent, fontSize: 13, fontWeight: '600' },
    sectionTitle: { fontSize: 16, fontWeight: '700', color: c.text, marginTop: 18, marginBottom: 10 },
    // Letras sobre los segmentos de color (COLOR_BG, contenido MTG): fondo fijo, texto fijo.
    flagRow: { flexDirection: 'row', borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: c.border },
    ...
    statLine: { fontSize: 15, color: c.textBody, marginBottom: 6 },
    winRateBox: {
      borderWidth: 1,
      borderColor: c.status.warning.text,
    ...
    winRateLabel: { fontSize: 9, color: c.status.warning.text, fontWeight: '600' },
    winRateValue: { fontSize: 14, color: c.status.warning.text, fontWeight: '800' },
    ...
    secondaryBtnTxt: { color: c.textBody, fontSize: 14, fontWeight: '600' },
    primaryBtn: {
      backgroundColor: c.accent,
      borderRadius: 8,
      paddingVertical: 10,
    ...
    primaryBtnTxt: { color: c.onAccent, fontSize: 14, fontWeight: '600' },
    leftEventText: { color: c.textSecondary, fontSize: 13, fontWeight: '600' },
    muted: { color: c.textSecondary, fontSize: 15 },
    stars: { fontSize: 22, color: c.status.warning.solid, marginBottom: 14, letterSpacing: 2 },
    ...
    h2hNameSide: { flex: 1, fontSize: 13, fontWeight: '700', color: c.text },
    ...
    h2hVs: { fontSize: 12, fontWeight: '700', color: c.textSecondary, flexShrink: 0 },
    tiebreakVsRow: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
    ...
    tiebreakH2hOpponentName: { fontSize: 13, fontWeight: '700', color: c.text },
    tiebreakH2hScore: {
      fontSize: 14,
      fontWeight: '800',
      color: c.text,
    ...
    tiebreakH2hWinner: { fontSize: 12, fontWeight: '600', color: c.status.success.text, marginTop: 4 },
    h2hDrawLabel: { fontSize: 13, fontWeight: '700', color: c.textSecondary, marginTop: 6, textAlign: 'center' },
    tiebreakProfileRoundTitle: { fontSize: 15, fontWeight: '800', color: c.text, marginTop: 12, marginBottom: 8 },
    h2hBo3Outer: {
      flexDirection: 'row',
      justifyContent: 'space-between',
      alignItems: 'center',
    ...
    bo3FilledBlue: { backgroundColor: c.accent, borderColor: c.accent },
    bo3FilledGreen: { backgroundColor: c.status.success.solid, borderColor: c.status.success.solid },
    bo3FilledOrange: { backgroundColor: c.status.warning.solid, borderColor: c.status.warning.solid },
    ...
    profilePhaseSubtitleOrange: { color: c.status.warning.text },
    revengeH2hRow: {
      flexDirection: 'row',
      alignItems: 'center',
      gap: 6,
    ...
    revengeH2hNameLeft: { fontSize: 13, fontWeight: '700', color: c.text, flexShrink: 1 },
    revengeH2hScore: {
      flexShrink: 0,
      fontSize: 14,
      fontWeight: '800',
    ...
    revengeH2hNameRight: { fontSize: 13, fontWeight: '700', color: c.text, flexShrink: 1, textAlign: 'right' },
    cupInsignia: {
    ...
    cupInsigniaTxt: { fontSize: 11, fontWeight: '700', color: c.status.info.text },
